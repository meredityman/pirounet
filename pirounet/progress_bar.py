"""
from eps.progress_bar import ProgressBar

pbar = ProgressBar(steps)

- single pbar
pbar.upd()
- single pbar with epochs
pbar.upd_laps(lap_steps=lap_steps, add_step=0)
- separate pbars for epochs
pbar.upd_laps(glob_start=glob_start, glob_step=glob_step, glob_steps=glob_steps, add_step=0)
"""

import os
import sys
import math
os.system('') #enable VT100 Escape Sequence for WINDOWS 10 Ver. 1607
# import colorama
# colorama.init()
import warnings
warnings.filterwarnings("ignore")

from shutil import get_terminal_size
import time

class ProgressBar(object):
    '''A progress bar which can print the progress
    modified from https://github.com/hellock/cvbase/blob/master/cvbase/progress.py
    '''
    def __init__(self, count=0, start_num=0, bar_width=50, start=True):
        self.task_num = count - start_num
        max_bar_width = self._get_max_bar_width()
        self.bar_width = (bar_width if bar_width <= max_bar_width else max_bar_width)
        self.completed = 0
        self.start_num = start_num
        if start:
            self.start()

    def _get_max_bar_width(self):
        terminal_width, _ = get_terminal_size()
        max_bar_width = min(int(terminal_width * 0.6), terminal_width - 50)
        if max_bar_width < 10:
            print('terminal is small ({}), make it bigger for proper visualization'.format(terminal_width))
            max_bar_width = 10
        return max_bar_width

    def start(self, task_num=None, start_num=None):
        if task_num is not None:
            self.task_num = task_num
        if start_num is not None:
            self.start_num = start_num
        if self.task_num > 0:
            sys.stdout.write('[{}] {}/{} \n{}\n'.format(' ' * self.bar_width, self.start_num, self.task_num + self.start_num, 'Start...'))
        else:
            sys.stdout.write('completed: 0, elapsed: 0s')
        sys.stdout.flush()
        self.start_time = time.time()

    def upd(self, msg=None, uprows=0):
        self.completed += 1
        elapsed = time.time() - self.start_time + 0.0000000000001
        fps = self.completed / elapsed if elapsed>0 else 0
        if self.task_num > 0:
            percentage = self.completed / float(self.task_num)
            eta = int(elapsed * (1 - percentage) / percentage + 0.5)
            finaltime = time.asctime(time.localtime(self.start_time + self.task_num * elapsed / float(self.completed)))
            fin_msg = ' %ss left, end %s' % (shortime(eta), finaltime[11:16])
            if msg is not None: fin_msg += '  ' + str(msg)
            mark_width = int(self.bar_width * percentage)
            bar_chars = 'X' * mark_width + '-' * (self.bar_width - mark_width) # ▒ ▓ █
            # sys.stdout.write('\033[2A') # cursor up 2 lines
            sys.stdout.write('\033[%dA' % (uprows+2)) # cursor up 2 lines + extra if needed
            sys.stdout.write('\033[J')  # clean the output (remove extra chars since last display)
            try:
                sys.stdout.write('[{}] {}/{}, rate {:.3g}s, time {}s, left {}s \n{}\n'.format(
                    bar_chars, self.completed + self.start_num, self.task_num + self.start_num, 1./fps, shortime(elapsed), shortime(eta), fin_msg))
            except:
                sys.stdout.write('[{}] {}/{}, rate {:.3g}s, time {}s, left {}s \n{}\n'.format(
                    bar_chars, self.completed + self.start_num, self.task_num + self.start_num, 1./fps, shortime(elapsed), shortime(eta), '<< unprintable >>'))
        else:
            sys.stdout.write('completed {}, time {}s, {:.1f} steps/s'.format(self.completed + self.start_num, int(elapsed + 0.5), fps))
        sys.stdout.flush()

    def upd_laps(self, glob_start=None, glob_step=None, glob_steps=None, lap_steps=None, add_step=0, msg=None):
        # single pbar :: just counts laps (doesn't calc lap time!)
        # args :: lap_steps // init count = total count!
        if glob_start is None: glob_start = self.start_time
        if glob_steps is None: glob_steps = self.task_num
        if glob_step is None: glob_step = self.completed
        # separate pbars :: calc both lap's and total time
        # args :: glob_start, glob_step, glob_steps
        if lap_steps is None: lap_steps = self.task_num
        glob_step += 1 # self will be updated below
        # calc stuff
        rate = (time.time() - glob_start) / float(glob_step)
        finaltime = time.asctime(time.localtime(glob_start + glob_steps*rate))
        left_sec = (glob_steps - glob_step) * rate
        lap = (glob_step + add_step) // lap_steps 
        if not glob_step % lap_steps == 0: lap += 1
        lap_count = (glob_steps  + add_step) // lap_steps
        msg_total = ' .. lap %d/%d global step %d .. %ss .. %ss left, end %s' % \
            (lap, lap_count, glob_step+add_step, shortime(time.time()-glob_start), shortime(left_sec), finaltime[11:16])
        if msg is not None:
            msg_total = msg + ' .. ' + msg_total
        step = self.upd(msg_total)
        return self.completed

    def reset(self, start_num=0, count=None, newline=False):
        self.start_time = time.time()
        self.start_num = start_num
        if count is not None:
            self.task_num = count - start_num
        if newline is True:
            sys.stdout.write('\n\n')

def shortime(sec):
    if sec < 60:
        time_short = '%d' % (sec)
    elif sec < 3600:
        time_short  = '%d:%02d' % ((sec/60)%60, sec%60)
    elif sec < 86400:
        time_short  = '%d:%02d:%02d' % (sec/3600, (sec/60)%60, sec%60)
    else:
        time_short = '%dd %d:%02d:%02d' % (sec/86400, (sec/3600)%24, (sec/60)%60, sec%60)
    return time_short


progbar = ProgressBar
